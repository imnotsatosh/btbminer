					  ...
					  int *curl_err, int flags)
{
	...
	applog(LOG_DEBUG, "JSON-RPC call: url:%s", url);
	applog(LOG_DEBUG, "JSON-RPC call: userpass:%s", userpass);
	...
	applog(LOG_DEBUG, "data received: %s", all_data.buf);
	if (curl_err != NULL)
		*curl_err = rc;
	if (rc)
	{
		...
		if (!((flags & JSON_RPC_LONGPOLL) && rc == CURLE_OPERATION_TIMEDOUT) &&
			...
			applog(LOG_ERR, "HTTP request failed: %s, err: %s", curl_err_str, curl_easy_strerror(rc));
