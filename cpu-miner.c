...
static bool gbt_work_decode(const json_t *val, struct work *work)
{
	...
	applog(LOG_INFO, "get version : %d", version);
	...
	if (coinbase_append)
	{
		...
		if (xsig_len)
		{
			...
			int push_len = cbtx[41] + xsig_len < 76 ? 1 : cbtx[41] + 2 + xsig_len > 100 ? 0
																						: 2;
			n = xsig_len + push_len;
	...
	txs_end = work->txs + strlen(work->txs);
...
static bool submit_upstream_work(CURL *curl, struct work *work)
{
	...
	else if (work->txs)
	{
		...
		applog(LOG_INFO, "submit block: %s", req);
